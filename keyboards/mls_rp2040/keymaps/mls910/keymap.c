...
const uint16_t PROGMEM encoder_map[][NUM_ENCODERS][NUM_DIRECTIONS] = {
    [_STD]        = {ENCODER_CCW_CW(ENC_STRIDE_INC,ENC_STRIDE_DEC),   // custom function for encoder turn being up & dn
...
void SleepMilliseconds(int msec) {
    static uint32_t a_timer;
    a_timer = timer_read();
    while ( timer_elapsed32(a_timer) < msec){
    }
  }
...
void do_delay_countable_wh_u(bool pressed){
    uint32_t t_d = 50;
    if (!pressed) { // key released: stop repeating.
        unregister_code(KC_WH_U);
        rpt_pos();
    } else if (pressed) { // key pressed: start repeating.
        SleepMilliseconds(t_d * stride);
...
void do_delay_countable_wh_d(bool pressed){
    uint32_t t_d = 50;
    if (!pressed) { // key released: stop repeating.
    ...
    */
    ...
    switch (keycode) {
        case EXP_WH_U:
            ...
            do_delay_countable_wh_u(record->event.pressed);
        ...
        case EXP_WH_D:
            ...
            do_delay_countable_wh_d(record->event.pressed);
            rpt_position_etc();  // update oled display for position etc.
            break;
