...
enum custom_keycodes {
    ...
    STD_WH_U,           // Standard QMK mouse wheel up
    STD_WH_D            // Standard QMK mouse wheel dn
};
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    [_BASE]        = LAYOUT(TO(_3SPEEDACL),                       // encoder press goto _3SPEEDACL
                            ...
                            ENC_U, STD_WH_U, ENC_STRIDE_INC,        // 1 shot u click stride x,reg. wheel,inc stride
                            ENC_D, STD_WH_D, ENC_STRIDE_DEC         // 1 shot d click stride x,reg. wheel,inc stride
                             ),
    ...
    switch (keycode) {
        ...
        count of the number sent, so the en_turns is not updated.*/
            if (record->event.pressed) {
                ...
                oled_clean_ln(4);
                oled_clean_write_ln(3, 5, "+ Scroll In +");
            } else {
                // when released
